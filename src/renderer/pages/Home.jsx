...
const EMULATORS = [
  { id:'pcsx2',   label:'PlayStation 2', icon:'ًںژ®', color:'#00439c', sub:'PCSX2' },
  { id:'dolphin', label:'GameCube / Wii', icon:'ًںگ¬', color:'#6b3fa0', sub:'Dolphin' },
  { id:'ppsspp',  label:'PlayStation Portable', icon:'ًں“±', color:'#1a6b3c', sub:'PPSSPP' },
];
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-8 gap-10 animate-fade-in">
        ...
        <div className="text-center">
          <h1 className="text-5xl font-black tracking-tight mb-2 bg-gradient-to-r from-white via-nexus-accent to-nexus-blue bg-clip-text text-transparent">
            ط§ظ„ط¹ط¨ ظ…ط¹ ط£طµط¯ظ‚ط§ط¦ظƒ
          </h1>
          <p className="text-white/40 text-sm">ط¨ط¯ظˆظ† ط³ظٹط±ظپط±ط§طھ آ· ط¨ط¯ظˆظ† ط§ط´طھط±ط§ظƒط§طھ آ· ط§طھطµط§ظ„ ظ…ط¨ط§ط´ط±</p>
        </div>

        {/* ط§ط®طھظٹط§ط± ط§ظ„ظ…ط­ط§ظƒظٹ */}
        <div className="flex gap-4">
          {EMULATORS.map((e, i) => (
            <button key={e.id}
              ...
              {selectedEmulator === e.id && (
                <div className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-nexus-accent flex items-center justify-center text-[10px]">âœ“</div>
              )}
            </button>
          ))}
        </div>
        ...
        <div className="flex gap-6 items-end">
          <button onClick={handleHost}
            ...
            ًںڈ  ط§ط³طھط¶ط§ظپط© ط؛ط±ظپط©
          </button>

          <div className="flex flex-col gap-2">
            <input value={joinCode}
                   onChange={e => setJoinCode(e.target.value.toUpperCase())}
                   placeholder="ظƒظˆط¯ ط§ظ„ط؛ط±ظپط©"
                   maxLength={6}
        ...
        <div className="text-white/30 text-xs text-center">
          ط¹ظ†ظˆط§ظ†ظƒ ط§ظ„ظ…ط­ظ„ظٹ: <span className="font-mono text-white/50">{localIP}</span>
        </div>
      </main>
    </div>
  );
